...
class moonPhase
{
	...
	//29.53058867
	var $periodInDays = 29.534027777778; // == complete moon cycle
...
*/
	...
	{
		//$curr = time();
		//$fullMoon1 = strtotime("December 12 2008 16:37 UTC");
		//$dif = time() - $fullMoon1;
		//$diff = $curr - $this->getBaseFullMoonDate();
		$diff = $this->dateAsTimeStamp - $this->getBaseFullMoonDate();
		//echo "<br/>" . $fullMoon1;
		//echo "<br/>" . time();
		//echo $this->someFullMoonDate;
		//echo "<br/>" . $diff . "<br/>";
		//echo "<br/> dateasts" . $this->getDateAsTimeStamp();
		//echo "<br/> basefullmoon" . $this->getBaseFullMoonDate();
		//echo "<br/>diff" . $diff;
		//echo date("Y-m-d H:i:s", $this->getBaseFullMoonDate()) . "<br/>";
		$periodInSeconds = $this->getPeriodInSeconds();
		//echo "<br/>per" . $periodInSeconds;
		$position = ($diff % $periodInSeconds) / ($periodInSeconds);
		//$position = ($diff / $periodInSeconds) - 1;
		/* echo "<br/>full" . $this->getBaseFullMoonDate() . "<br/>";
		echo "<br/>date" . $curr . "<br/>";
		echo "<br/>diff" . $diff . "<br/>";
		echo "<br/>per" . $periodInSeconds . "<br/>";
		echo "<br/>rez" . ($diff - $periodInSeconds) . "<br/>"; */
		if ($position < 0)
			$position += 1;
		return $position;
...
*/
	function getUpcomingWeekArray($newStartingDateAsTimeStamp = -1)
	{
		...
		for (
			...
			$weeklyPhase->setDate1($thisTimeStamp);
